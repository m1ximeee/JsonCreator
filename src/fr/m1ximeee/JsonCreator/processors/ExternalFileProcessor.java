...
import fr.m1ximeee.JsonCreator.Object.ExternalFile;
        ...
        if(dir.listFiles() != null)
            for (File extFile : dir.listFiles())
                if(!extFile.isDirectory())
                    this.externalFiles.add(new ExternalFile(extFile.getName(), MainPanel.url.getText() + extFile.getName(), FileUtils.getSHA1(extFile), FileUtils.getFileSizeBytes(extFile)));
                else
                {
                    for (File sub : this.getSubFiles(extFile))
                        this.externalFiles.add(new ExternalFile(sub.getAbsolutePath().replace(dir.getAbsolutePath() + File.separator, ""), MainPanel.url.getText() + sub.getAbsolutePath().replace(dir.getAbsolutePath() + "/", ""), FileUtils.getSHA1(sub), FileUtils.getFileSizeBytes(sub)));
                }
    }
